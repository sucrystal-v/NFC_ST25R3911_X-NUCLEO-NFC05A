...
bool demoIni( void )
{
    ...
    if( err == ERR_NONE )
    {
        discParam.compMode      = RFAL_COMPLIANCE_MODE_NFC;
				discParam.devLimit      = 10;					//CL: maximum device aloowed; need to change also RFAL_NFC_MAX_DEVICES 

        discParam.notifyCb             = demoNotif;
        discParam.totalDuration        = 1000U;
        discParam.wakeupEnabled        = false;
        discParam.wakeupConfigDefault  = true;
        discParam.techs2Find           = (  RFAL_NFC_POLL_TECH_V ); //CL: Poll olny T5T
                    ...
                    case RFAL_NFC_LISTEN_TYPE_NFCV:
                        {
                            uint8_t devUID[RFAL_NFCV_UID_LEN];
                     				uint8_t i;                            
														for(i=0; i<devCnt; i++)		//CL: list all the t5t device's UID
														{	
			       
															ST_MEMCPY( devUID, nfcDevice->dev.nfcv.InvRes.UID, RFAL_NFCV_UID_LEN);   /* Copy the UID into local var */
															REVERSE_BYTES( devUID, RFAL_NFCV_UID_LEN );                 /* Reverse the UID for display purposes */
															platformLog("ISO15693/NFC-V card no.%d found. UID: %s\r\n", i+1 ,hex2Str(devUID, RFAL_NFCV_UID_LEN));
...
static void demoNfcv( rfalNfcvListenDevice *nfcvDev )
{
    ReturnCode            err;
    uint16_t              rcvLen;
    uint8_t               blockNum = 0x1;
    uint8_t               rxBuf[ 1 + DEMO_NFCV_BLOCK_LEN + RFAL_CRC_LEN ];                        /* Flags + Block Data + CRC */
    uint8_t *             uid; 
#if DEMO_NFCV_WRITE_TAG
    uint8_t               wrData[DEMO_NFCV_BLOCK_LEN] = { 0x11, 0x22, 0x33, 0x44 };             /* Write block example */

 ...
 ******************************************************************************
 */
static uint8_t ndefWifiName[] = "ST network"; //CL: type the wifi name 
static uint8_t ndefWifiKey[] = "00000000";  //CL: type the wifi password 
...
/////////////////////////////////////////Wifi////////////////////////////////////////////////////////////////////////
		...
		myWifi.bufNetworkSSID.buffer = ndefWifiName;
		myWifi.bufNetworkSSID.length = sizeof(ndefWifiName) - 1U;
		myWifi.bufNetworkKey.buffer = ndefWifiKey;
		myWifi.bufNetworkKey.length = sizeof(ndefWifiKey) - 1U;
		myWifi.authentication =0x20; //NDEF_WIFI_AUTHENTICATION_WPA2PSK		
		myWifi.encryption = 0x08; //NDEF_WIFI_ENCRYPTION_AES
		
    err |= ndefWifi(&wifi, &myWifi); /* Initialize Wifi type structure */
    err |= ndefWifiToRecord(&wifi, &record1); /* Encode Wifi record */

    err |= ndefMessageAppend(&message, &record1); /* Append Wifi to message */
	...
	platformLog("Wrote Wifi Pairing Ndef record to the Tag\r\n");
	return;  
}

...
static void demoNfcv( rfalNfcvListenDevice *nfcvDev )
{
    ...
    uint8_t *             uid; 
		uint8_t 							pwdNum=0;
		uint8_t 							newpwd[8]={0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00}; //CL: for ST25DV, pwd is 8 bytes
		uint8_t 							oldpwd[8]={0x99,0x99,0x99,0x99,0x99,0x99,0x99,0x99};     
		
    uid = nfcvDev->InvRes.UID;
    
		errPre = rfalST25xVPollerPresentPassword( RFAL_NFCV_REQ_FLAG_DEFAULT, uid, pwdNum, oldpwd, sizeof(oldpwd)); //CL: before changing the static register, need to present firstly the PWD.
		if(errPre==ERR_NONE)
		{
			err = rfalST25xVPollerWritePassword( RFAL_NFCV_REQ_FLAG_DEFAULT, uid, pwdNum, newpwd, sizeof(newpwd)); //CL: before changing the static register, need to present firstly the PWD.		
			platformDelay(5);
			if(err==ERR_NONE)
			{		
				platformLog("Password changed sucessfully!\n");	
			}		
			else
			{
				platformLog("Password can not be changed with error code %d!\n",err);	
			}			
		}
		else
		{
			platformLog("Original password is not correct with error code %d!\n",errPre);	
		}
}
...
/*! 
 *****************************************************************************
 * \brief  NFC-V Poller Write Password format
 *  
 * Sends ST25xV Poller Write Password command  */
ReturnCode rfalST25xVPollerWritePassword( uint8_t flags, const uint8_t* uid, uint8_t pwdNum, const uint8_t *pwd,  uint8_t pwdLen)
{
		uint8_t 					 ST25xV_PWDNUM_LEN=1U;   /*!< Password Number length                                            */
		uint8_t 					 ST25xV_PWD_LEN = 8U;   /*!< Password length   */   
		uint8_t            data[ST25xV_PWDNUM_LEN + ST25xV_PWD_LEN];
    uint8_t            dataLen;
    uint16_t           rcvLen;
    rfalNfcvGenericRes res;
    
    if( (pwdLen > ST25xV_PWD_LEN) || (pwd == NULL) )
    {
        return ERR_PARAM;
    }
